unsigned reverse32(register unsigned x)
{
	x = (((x & 0xAAAAAAAA) >> 1) | ((x & 0x55555555) << 1));
	x = (((x & 0xcccccccc) >> 2) | ((x & 0x33333333) << 2));
	x = (((x & 0xF0F0F0F0) >> 4) | ((x & 0x0F0F0F0F) << 4));
	x = (((x & 0XFF00FF00) >> 8) | ((x & 0X00FF00FF) << 8));
	return (x >> 16) | (x << 16);
}

/* Courtesy of matt-j@stackoverflow.com */
unsigned reverse32_fast(unsigned x)
{
	static const unsigned char revtable[] =
	{
		
	};
	return	(revtable[x & 0xFF] << 24) |
		(revtable[(x >> 8) & 0xFF] << 16) |
		(revtable[(x >> 16) & 0xFF] << 8) |
		(revtable[(x >> 24) & 0xFF]);
}


/* Debugging stuff below this line. Not beautiful code */

void printbin(unsigned n)
{
	unsigned long long i;
	int j;
	char s[33];
	s[32] = '\0';
	for (i = 1, j = 31; i <= 4294967295; i *= 2)
		s[j--] = n & i ? '1' : '0';
	printf("%s", s);
}
